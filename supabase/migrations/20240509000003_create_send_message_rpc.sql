-- Migration: Create RPC function for users to send messages
...
BEGIN
    ...
    -- Optional: Check if users are connected before allowing messaging (depends on product requirements)
...
IS 'Allows an authenticated user to send a message to another active user. Returns the created message.';

DO $$ BEGIN
  RAISE NOTICE 'RPC function send_message created and granted.';
END $$; 